...
public class OdometryCorrection implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			// all numbers within if-condition correspond to range of angle (in degrees) on
			// square corners
			if (intensity < 0.3) {
				Sound.beep();
				if ((odometer.getXYT()[2] >= 350 && odometer.getXYT()[2] <= 360)
						|| (odometer.getXYT()[2] >= 0.00 && odometer.getXYT()[2] <= 10)) { // case 1: keep increasing
																							// y-position until wheels
																							// turn 90 degrees
					odometer.setY(county * size); // robot is horizontally in line with origin
					county++;
				} else if (odometer.getXYT()[2] >= 80 && odometer.getXYT()[2] <= 100) { // case 2: keep increasing
																						// x-position until 90 degree
																						// turn
					odometer.setX(countx * size);
					countx++;
				} // robot is vertically in line with origin
				else if (odometer.getXYT()[2] >= 170 && odometer.getXYT()[2] <= 190) { // case 3: keep decreasing
																						// y-position until 90 degree
																						// turn
					county--;
					odometer.setY(county * size);
					bufferY = odometer.getXYT()[1];
				} else if (odometer.getXYT()[2] >= 260 && odometer.getXYT()[2] <= 284) { // case 4: keep decreasing
																							// x-position until 90
																							// degree turn
					countx--;
					odometer.setX(countx * size);
